...
const findHentaiElement = async url => {
  const driver = await creatDriver()
  const urls = []
  await driver.get(url).then(async () => {
    let containers = await driver.wait(until.elementsLocated(By.className('CapImagePage')), 2000)
    const len = containers.length
    for (let i = 0; i < len; i++) {
      const item = containers[i]
      const imgElements = await item.findElement(By.css('img'))
...
class BcyDao {
  ...
  async getHentaiCosplay () {
    return findHentaiElement(this.data)
  }
}
